...
function preparationFlow($helper, $args, $commandData, $streamTotalSec = 0, $autoPin = null, $setUser = null, $charity = null, $title = null)
{
    ...
    try {
        ...
        Utils::log("\n\n\e[93m[!] Recovery:\e[0m Detected recovery was outdated, deleting...");
        Utils::deleteRecovery();
        Utils::log("\e[91m[x] Recovery:\e[0m Deleted Outdated Recovery!\e[0m");
    }
    ...
    if (Utils::isRecovery() && !acceptRecovery) {
        Utils::log("\n\n\e[93m[!] Recovery:\e[0m Detected a previous stream that exited improperly! Would you like to pick up where you left off or start from scratch?");
        Utils::log("\e[93m[?] Recovery\e[0m: Type\"yes\" to pick up where you left off or enter to start from scratch...");
    ...
    try {
        ...
        if (!Utils::isRecovery()) {
            if (($helper->obs_path === null) && !disableObsMsgs) {
                ...
                if (!obsAutomationAccept && !disableObsMsgs) {
        ...
        if (!Utils::isRecovery()) {
            ...
            Utils::log("\e[96m[✓] Livestream: Started livestream!\e[0m");
...
function livestreamingFlow($ig, $broadcastId, $streamUrl, $streamKey, $obsAuto, $helper, $streamTotalSec, $autoPin, $args, $commandData, $startCommentTs = 0, $startLikeTs = 0, $startingQuestion = -1, $startingTime = -1)
{
    ...
    do {
        ...
        if (webMode) {
            file_put_contents('webLink.json', json_encode([
