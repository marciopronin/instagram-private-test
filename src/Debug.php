...
class Debug
{
    ...
    public static function printRequest(
        ...
        if (self::$debugLog && ($path !== null)) {
            file_put_contents(sprintf('%s/%s-%s', $path, date('d-m-Y'), self::$debugLogFile), date('d-m-Y H:i:s').'  '.$method.':  '.$endpoint."\n", FILE_APPEND | LOCK_EX);
        }
    }
    ...
    public static function printUpload(
        ...
        if (self::$debugLog && ($path !== null)) {
            file_put_contents(sprintf('%s/%s-%s', $path, date('d-m-Y'), self::$debugLogFile), "→  $uploadBytes\n", FILE_APPEND | LOCK_EX);
        }
    }
    ...
    public static function printHttpCode(
        ...
        if (self::$debugLog && ($path !== null)) {
            file_put_contents(sprintf('%s/%s-%s', $path, date('d-m-Y'), self::$debugLogFile), "← {$httpCode} \t {$bytes}\n", FILE_APPEND | LOCK_EX);
        }
    }
    ...
    public static function printResponse(
        ...
        if (self::$debugLog && ($path !== null)) {
            file_put_contents(sprintf('%s/%s-%s', $path, date('d-m-Y'), self::$debugLogFile), "RESPONSE: {$response}\n\n", FILE_APPEND | LOCK_EX);
        }
    }
    ...
    public static function printEvent(
        ...
        if (self::$debugLog && ($path !== null)) {
            file_put_contents(sprintf('%s/%s-%s', $path, date('d-m-Y'), self::$debugLogFile), $event."\n".$eventBatch."\n", FILE_APPEND | LOCK_EX);
        }
    }
}
